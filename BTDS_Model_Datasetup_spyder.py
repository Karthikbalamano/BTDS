# -*- coding: utf-8 -*-
"""
Created on Thu Aug 25 19:32:14 2022
...
test_image_flair=nib.load(TRAIN_DATASET_PATH + 'BraTS20_Training_010/BraTS20_Training_010_flair.nii').get_fdata()
print(test_image_flair.max())
#Scalers are applied to 1D so let us reshape and then reshape back to original shape. 
test_image_flair=scaler.fit_transform(test_image_flair.reshape(-1, test_image_flair.shape[-1])).reshape(test_image_flair.shape)


test_image_t1=nib.load(TRAIN_DATASET_PATH + 'BraTS20_Training_010/BraTS20_Training_010_t1.nii').get_fdata()
test_image_t1=scaler.fit_transform(test_image_t1.reshape(-1, test_image_t1.shape[-1])).reshape(test_image_t1.shape)

test_image_t1ce=nib.load(TRAIN_DATASET_PATH + 'BraTS20_Training_010/BraTS20_Training_010_t1ce.nii').get_fdata()
test_image_t1ce=scaler.fit_transform(test_image_t1ce.reshape(-1, test_image_t1ce.shape[-1])).reshape(test_image_t1ce.shape)

test_image_t2=nib.load(TRAIN_DATASET_PATH + 'BraTS20_Training_010/BraTS20_Training_010_t2.nii').get_fdata()
test_image_t2=scaler.fit_transform(test_image_t2.reshape(-1, test_image_t2.shape[-1])).reshape(test_image_t2.shape)

test_mask=nib.load(TRAIN_DATASET_PATH + 'BraTS20_Training_010/BraTS20_Training_010_seg.nii').get_fdata()
...
imsave('BraTS2020_TrainingData/combined010.tif', combined_x)
np.save('BraTS2020_TrainingData/combined010.npy', combined_x)
#Verify image is being read properly
#my_img=imread('BraTS2020_TrainingData/combined010.tif')

my_img=np.load('BraTS2020_TrainingData/combined010.npy')
